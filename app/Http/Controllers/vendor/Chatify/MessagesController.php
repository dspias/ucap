...
class MessagesController extends Controller
{
    ...
    {
        $path = storage_path() . '/app/media/' . config('chatify.attachments.folder') . '/' . $fileName;
        if (file_exists($path)) {
        ...
        if ($request->hasFile('file')) {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed)) {
                    ...
                    $file->storeAs("media/" . config('chatify.attachments.folder'), $attachment);
                } else {
            ...
            if ($file->getSize() < 150000000) {
                if (in_array($file->getClientOriginalExtension(), $allowed_images)) {
                    ...
                    if (Auth::user()->avatar != config('chatify.user_avatar.default')) {
                        $path = storage_path('app/media/' . config('chatify.user_avatar.folder') . '/' . Auth::user()->avatar);
                        if (file_exists($path)) {
                            @unlink($path);
                        }
                    }
                    ...
                    $file->storeAs("media/" . config('chatify.user_avatar.folder'), $avatar);
                    $success = $update ? 1 : 0;
                } else {
