...
@section('page_title', __('Inactive Representatives'))
...
@section('page_name')
    <b class="text-uppercase">{{ __('Inactive Representatives') }}</b>
@endsection


@section('breadcrumb')
    <li class="breadcrumb-item">{{ __('Representatives') }}</li>
    <li class="breadcrumb-item Inactive">{{ __('Inactive Representatives') }}</li>
@endsection


@section('action')
    <a class="btn btn-custom bg-ucap" href="{{ route('university.representative.create') }}">
                                        ...
                                        <div class="btn-group mr-2">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="btn_id_no_01">
                                                    @if($data->status == 1)
                                                    <a class="dropdown-item text-danger confirm" confirm="Are You sure to inactive this representative?" href="{{ route('university.representative.change_status', [$data->id]) }}">
                                                        <i class="feather icon-x text-danger"></i> {{ __('Inactive') }}
                                                    </a>
                                                    @else
                                                    <a class="dropdown-item text-success confirm" confirm="Are You sure to active this representative?" href="{{ route('university.representative.change_status', [$data->id]) }}">
                                                        <i class="feather icon-check text-success"></i> {{ __('Active') }}
                                                    </a>
                                                    @endif
                                                    <a class="dropdown-item" href="{{ route('university.representative.edit', [$data->id]) }}">
                                                        <i class="feather icon-edit-2"></i> {{ __('Edit') }}
                                                    </a>
                                                    <a class="dropdown-item" href="{{ route('university.representative.show', ['page' => 'all', 'id' => $data->id]) }}">
