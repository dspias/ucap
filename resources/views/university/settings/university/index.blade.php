...
@section('page_name')
    <b class="text-uppercase">{{ $university->name }}</b>
@endsection
...
@section('action')
    ...
    <a href="{{ route('guest.university.show', ['id' => decbin($university->id), 'name' => get_name($university->name)]) }}" target="_blank" class="btn btn-custom bg-ucap" data-toggle="tooltip" data-title="{{ __('See preview in live website') }}">
...
<div class="row">
    ...
    <div class="col-md-12">
        <div class="card m-b-30">
            <form action="{{ route('university.settings.university.update') }}" method="post" enctype="multipart/form-data">
                @csrf
