...
@section('page_title', __('Administrator | Details'))
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('UCAP Admins') }}</li>
...
@section('action')
    ...
    <a href="{{ route('superadmin.administrator.edit', [$data->id]) }}" class="btn btn-custom bg-ucap confirm" confirm="Are you sure want to edit profile?">
        ...
        <div class="card m-b-30">
            <div class="card-body">
                <div class="profilebox py-4 text-center">
                    @if(!is_null($url = has_media($data->superadmin, 'avatar', 'thumb')))
                    ...
                    <div class="button-list">
                        <a href="Tel: {{ optional($data->superadmin)->phone }}" class="btn btn-primary-rgba font-18"><i class="feather icon-phone"></i></a>
                    ...
                    <table class="table table-bordered">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-right">{{ get_date(optional($data->superadmin)->dob, 'd M, Y') }} <sup class="text-ucap text-bold">{{ get_age(optional($data->superadmin)->dob).' Years' }}</sup></td>
                            </tr>
                            <tr>
                                <th class="text-left">{{ __('Gender') }}</th>
                                <td class="text-right">{{ optional($data->superadmin)->gender }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-right">{{ optional($data->superadmin)->phone }}</td>
                            </tr>
                            <tr>
                                <th class="text-left">{{ __('Native Language') }}</th>
                                <td class="text-right">{{ native_lang(optional($data->superadmin)->language) }}</td>
                            </tr>
                            <tr>
                                <th class="text-left">{{ __('Country') }}</th>
                                <td class="text-right">{{ optional($data->superadmin)->country }}</td>
                            </tr>
                            <tr>
                                <th class="text-left">{{ __('Province') }}</th>
                                <td class="text-right">{{ optional($data->superadmin)->state }}</td>
                            </tr>
                            <tr>
                                <th class="text-left">{{ __('City') }}</th>
                                <td class="text-right">{{ optional($data->superadmin)->city }}</td>
                            </tr>
                            <tr>
                                <th class="text-left">{{ __('Address') }}</th>
                                <td class="text-right">{{ optional($data->superadmin)->address }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
