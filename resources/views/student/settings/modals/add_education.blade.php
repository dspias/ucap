<div class="modal fade" id="addNewQualification" tabindex="-1" role="dialog" aria-labelledby="addNewQualification-label" aria-hidden="true">
    <div class="modal-dialog modal-lg" style="max-width: 800px;" role="document">
        <div class="modal-content">
            <div class="modal-header bg-dark">
                <h5 class="modal-title text-white" id="addNewQualification-label">{{ __('Your Education') }}</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form action="{{ route('student.settings.education.store') }}" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    @csrf
                    <div class="row">
                        <div class="form-group col-md-4">
                            <label>{{ __('Level of Education') }}<sup class="text-ucap">*</sup></label>
                            <select class="form-control" name="level" id="level" required style="height: 54px;">
                        ...
                        <div class="form-group col-md-4">
                            ...
                            {{-- <select class="form-control" name="field" id="field" required style="height: 54px;">
                                <option value="" aria-disabled="true">{{ __('Select Education Field') }} *</option>
                                <option value="education_field_name_here">{{ __('education_field_name_here') }}</option>
                            </select> --}}
                            <input type="text" class="form-control" name="field" id="field" placeholder="{{ __('Enter Program Name') }} *" required>
                        </div>

                        <div class="form-group col-md-4">
                            <label for="major_subject" class="col-form-label pt-0">{{ __('Major Subject') }}</label>
                            <input type="text" class="form-control" name="major_subject" id="major_subject" placeholder="{{ __('Enter Major Subject Name') }}">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="institute" class="col-form-label">{{ __('Institute') }}</label>
                        <input type="text" class="form-control" name="institute" id="institute" placeholder="{{ __('Enter Institute Name') }}">
                    </div>

                    <div class="form-group">
                        <label for="address" class="col-form-label">{{ __('Institute Address') }} <sup class="text-ucap">*</sup></label>
                        <input type="text" class="form-control" name="address" id="address" placeholder="{{ __('Enter Institute Address') }}" required>
                    </div>

                    <div class="row">
                        <div class="form-group col-md-4">
                            <label for="start_year" class="col-form-label">{{ __('Started At') }} <sup class="text-ucap">*</sup></label>
                            <input type="number" minlength="4" min="1990" maxlength="4" max="{{ date("Y") }}" class="form-control" name="start_year" id="start" placeholder="{{ __('2018') }}" required>
                        </div>

                        <div class="form-group col-md-4">
                            <label for="end_year" class="col-form-label">{{ __('Ends At') }} <sup class="text-ucap">*</sup></label>
                            <input type="number" minlength="4" min="1990" maxlength="4" max="{{ date("Y") }}" class="form-control" name="end_year" id="end_year" placeholder="{{ __('2020') }}" required>

                            <div class="sm-add-ship">
                                <input id="studying" class="checkbox-custom" name="studying" type="checkbox">
                                <label for="studying" class="checkbox-custom-label">{{ __('Studying Yet') }}</label>
                            </div>
                        </div>

                        <div class="form-group col-md-4">
                            <label for="score" class="col-form-label">{{ __('Passing Score') }} <sup class="text-ucap">*</sup></label>
                            <input type="text" class="form-control" name="score" id="score" placeholder="{{ __('4.50') }}" required>
                        </div>
